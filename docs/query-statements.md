...
- **Recursive CTEs** — `WithRecursive()` emits `WITH RECURSIVE`, required by MySQL (8.0+), PostgreSQL, and SQLite. Oracle — every version, 23ai included (live-verified) — and SQL Server reject the `RECURSIVE` keyword; on SQL Server, recurse with plain `With(...)` instead. Oracle additionally requires the CTE column list on a recursive body, which plain `With()` does not yet emit, so a recursive CTE that runs on Oracle is currently not expressible ([#348](https://github.com/h-tacayama/SqlArtisan/issues/348) tracks the gap). The analyzer warns when `WithRecursive()` targets a dialect (or a declared version) that does not support it. `WithRecursive()` always includes the CTE column list, derived from the first query block (`WITH RECURSIVE "cte"(code) AS (...)`) — Oracle-style recursive `WITH` requires the list, and every engine accepts it. Every select item of the CTE's first query block must therefore have a name: a plain column, or an expression aliased with `.As(...)` — an unnamed expression throws at the `WithRecursive()` call. Plain `With()` never emits a column list.
