...
Read a column back the way it was named. `Column("id")` renders the name bare
(`"x".id`), which matches a column the subquery projects bare — a plain column,
...
// USING users "s"
// ON ("t".id = "s".id)
// WHEN MATCHED THEN UPDATE SET name = "s".name WHERE "t".name <> "s".name
// WHEN NOT MATCHED THEN INSERT (id, name) VALUES ("s".id, "s".name) WHERE "s".name IS NOT NULL
```
