...
```csharp
UnnestDerivedTable t = Unnest(BindArray(new[] { 10, 20, 30 })).AsTable("v");
SqlStatement sql =
    Select(t.Column("v"))
    .From(t)
...
`:0` binds the whole `int[]` as one parameter. PostgreSQL only. `.AsTable(alias, columns...)` names the result columns instead — `UNNEST(:0, :1) "t" (x, y)` — one column per array, for unnesting several arrays in parallel. The argument is any array expression: a [`BindArray(...)` parameter](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/expressions.md#any--all-with-a-bound-array-postgresql), an `ARRAY[...]` constructor, or an array-typed column.
...
which the analyzer reports as `SQLA0104` — unlike `OrderBy`, which refuses a
non-integer sort key at `Build(Dbms)` for the dialects that reject it.

An integer inside `Rollup(...)`, `Cube(...)` or `GroupingSets(...)` is **not** a
position: those take expressions, so `GroupBy(Rollup(1, 2))` emits
`ROLLUP(:0, :1)` and groups by two bound constants. Pass the columns there.
...
> [!WARNING]
> **A negative computed offset means a different thing on each target.** MySQL,
> PostgreSQL and SQL Server reject the statement; Oracle executes it and SQLite
> reads it as 0 — so `Offset((page - 1) * size)` with `page` at 0 is a hard
> error on three targets and a silently wrong page on the other two. Nothing
> diagnoses it: the offset travels as a bind parameter, so `Build(Dbms)` never
> sees it, and the analyzer reads only call-site constants. Clamp the page
> number before computing it.
...
On PostgreSQL and SQL Server a `MERGE` can be fed by a CTE —
`With(cte.As(...)).MergeInto(target)...`, described under
...
- **Recursive CTEs** — `WithRecursive()` emits `WITH RECURSIVE`, required by
  MySQL and PostgreSQL and accepted by SQLite (MySQL gained CTE support in a
  specific release — see the
  [version-bound register](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#version-bound-constructs)).
  Oracle — every version, 23ai included (live-verified) — and SQL Server reject
  the `RECURSIVE` keyword; there, recurse with plain `With(...)`. On Oracle a
  recursive body additionally requires the CTE column list — chain
  `.WithColumnList()` onto the CTE binding
  (`With(cte.As(...).WithColumnList())`) to emit it
  (`WITH "cte"(code) AS (...)`); SQL Server recurses with plain `With(...)`
  alone, and also accepts the list. The analyzer warns when `WithRecursive()`
  targets a dialect (or a declared version) that does not support it.
  `WithRecursive()` always includes the CTE column list, derived from the first
  query block — Oracle-style recursive `WITH` requires the list, and every
  engine accepts it; `.WithColumnList()` derives the same list under plain
  `With()`. Every select item of the CTE's first query block must therefore have
  a name: a plain column, or an expression aliased with `.As(...)` — an unnamed
  expression throws at the `WithRecursive()` or `.WithColumnList()` call. Plain
  `With()` emits a column list only for CTEs that opt in with
  `.WithColumnList()`.
- **CTEs with DML main statements** — `With(...)` before an `UPDATE` or `DELETE`
  main statement is supported on MySQL, PostgreSQL, SQLite, and SQL Server,
  before an `INSERT` on PostgreSQL, SQLite, and SQL Server, and before a `MERGE`
  on PostgreSQL and SQL Server — MySQL and SQLite have no `MERGE` at all, so the
  CTE-fed form is out of reach there for want of the statement. Oracle's DML
  grammar has no leading `WITH`, and MySQL's `INSERT` grammar has none either
  ...
  `Build(Dbms.Oracle)` throws on the leading form. `WithRecursive(...)` opens
  each of these statements except `MERGE`, which no engine takes a recursive
  `WITH` before — PostgreSQL, the only engine with both the keyword and the
  statement, refuses the pairing whether or not the CTE body recurses
  (live-verified on 16) — so `WithRecursive(...).MergeInto(...)` does not
  compile. DML *inside* a CTE body (PostgreSQL data-modifying CTEs) is not
  supported.
...
On `INSERT` the column list is optional, and so is having one to follow: `InsertInto(t).Output(...).Values(...)` emits `INSERT INTO t OUTPUT ... VALUES (...)`, which SQL Server 2022 runs. What that state does not offer is `Set(...)` — it emits the column list, and T-SQL puts the column list ahead of `OUTPUT`, so the two have no spelling in that order. Write the columns out (`InsertInto(t, t.Id, t.Name).Output(...)`) when you want both; building both on one held builder throws.
