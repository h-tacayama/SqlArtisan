...
Every match-option, `GROUP BY` and row-count verdict above — the rejections
and the acceptances that keep the other engines silent — is checked against the
...
it covers all four value domains together; a `sqlartisan_construct_*` override
...
The matrix's `verified` entries, and the match-option, `GROUP BY` and row-count
facts [Argument value validity](#argument-value-validity-sqla0104) reports, were
...
- **Six constructs are entered per-overload only, with no member-wide
  verdict.** `Concat`, `Date`, `Grouping`, `GroupingId`, `IntervalLiteral` and
  `Log` have matrix entries for specific argument counts and no entry covering
  the member as a whole, so a call whose argument count isn't among them would
  never warn. Every overload these six ship today *is* entered (a test gates
  that), so the gap is not reachable from the current API — it is listed
  because the `sqlartisan_construct_<name>` member-level override still applies
  to all of them, while the shipped matrix's verdicts there are per-overload.
