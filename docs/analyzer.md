...
| `SQLA0007` | Warning | `IS NULL` / `IS NOT NULL` on a column the generated table class declares `NOT NULL`, so the predicate's answer is fixed before the query runs. Silent in any statement with an outer join, where the anti-join makes exactly this predicate meaningful — see [Schema-aware warnings](#schema-aware-warnings-sqla0007). |
...
A statement containing an **outer join** is never reported: past a
`LeftJoin` / `RightJoin` / `FullJoin` (or a lateral / `OuterApply` form), a
NOT NULL column is legitimately NULL on the null-supplied side, and
`.Where(r.Id.IsNull)` after a `LeftJoin` is the idiomatic anti-join. The rule
does not work out which side the join null-supplies — any outer join in the
statement silences it, trading missed warnings for none that are wrong.
...
The third is a row the engine cannot construct: an `INSERT` whose column list
leaves out a column that is `NOT NULL` and has no default.
...
**All three are silent unless the fact was recorded.** An attribute the generator
