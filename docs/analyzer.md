...
| `RegexpLike`, `RegexpCount`, `RegexpReplace`, `RegexpSubstr`, `RegexpInstr` | PostgreSQL | 15 | `regexp_like`, `regexp_count`, `regexp_substr`, and `regexp_instr` landed in PostgreSQL 15. `regexp_replace` predates it — 15 is where it gained the position and occurrence arguments — but the bound covers every `RegexpReplace` overload, so a 3-argument call is reported below 15 too. MySQL's `REGEXP_INSTR` tops out at 6 arguments (no `subexpr`), so `RegexpInstr`'s 7-argument overload is MySQL-unsupported at any version, independent of this PostgreSQL bound. |
