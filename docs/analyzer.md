...
(`.Where(s.Ref.IsNotNull)`). `IN` is unaffected — there a NULL merely fails to
match — so only `NOT IN` is reported.
...
**Both are silent unless the fact was recorded.** An attribute the generator
...
subquery held in a variable, or one selecting anything other than a single
column, is left alone. `SQLA0009` skips a statement whose column list it cannot
read in full — a column array built elsewhere — since a column it failed to
read would otherwise look omitted.
