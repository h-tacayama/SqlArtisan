...
| ID | Severity | Reports |
|---|---|---|
| `SQLA0001` | Warning | A SqlArtisan analyzer configuration problem: an unrecognized `sqlartisan_syntax_*` key name or value, a `sqlartisan_target_dbms` / `sqlartisan_construct_*` value that could not be recognized, a `sqlartisan_syntax_*` family that resolves to no dialect at all, or the legacy pair coexisting with a present `sqlartisan_syntax_*` key — see [Checking a set of dialects at once](#checking-a-set-of-dialects-at-once). |
...
```ini
# repo-wide: check every DBMS but SQLite
[*.cs]
sqlartisan_syntax_mysql      = any
...
# this one area also ships to SQLite
[src/EmbeddedReports/**.cs]
sqlartisan_syntax_sqlite = any
```

No aliases beyond these two — no `true`/`yes`/`off`. A smaller value domain
is easier to document and validate.
...
resolves to an empty set — every rule's "is a dialect configured" gate then
reads as "unconfigured," and the whole analyzer goes quiet. `SQLA0001`
reports this once per compilation when the family is present but every key
resolves to `none`, so a project doesn't lose analyzer coverage from one
typo-adjacent `none` without a visible reason.
...
fact *replaces* PostgreSQL. The message names the dropped DBMS and the key
that would keep it checked — this is a distinct `SQLA0001` report, not
`SQLA0002`; the two never fire for the same configuration.
