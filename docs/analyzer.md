...
```csharp
[DbColumnMetadata(Nullable = false, HasDefault = false, ColumnType = DbColumnType.Text)]
...
`Boolean` — carrying no length, precision, or scale. A `numeric(10,2)` column compared to an
