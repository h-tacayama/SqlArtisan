...
Every verdict in the two tables above — the rejections and the two acceptances
that keep Oracle and SQLite silent alike — is checked against the engine
versions in [Verified-against versions](#verified-against-versions).
...
- **This rule has no fact for the pair** — `Extract` targeting SQL Server,
  which `SQLA0100` already rejects outright; or an engine absent from the
  tables above, which is an engine nobody has measured, not a rejecting one.
