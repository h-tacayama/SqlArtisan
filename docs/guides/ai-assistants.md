...
- [Keep the table classes in step with the database](#keep-the-table-classes-in-step-with-the-database)
...
## Keep the table classes in step with the database

Table classes are what make an assistant's column names compile-checked, so a
class that has fallen behind the schema quietly removes that check. The
`SqlArtisan.TableClassGen` tool is fully argument-driven, which means an
assistant can regenerate and verify them itself whenever a database is
reachable (a `docker compose` development database, for example):
...
- Target DBMS: PostgreSQL. SqlArtisan emits SQL faithfully with no dialect
  ...
  SQLA0100 analyzer warnings as errors to fix, not suppress.
