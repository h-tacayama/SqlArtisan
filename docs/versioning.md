...
Four cases are specific to this library, beyond the usual API-level changes:
...
- **`SqlArtisan.Internal` is implementation detail.** A type there is `public`
  only where the fluent chain's own signatures reach it, so narrowing one to
  `internal` or reshaping it is **not** a breaking change. The exception is
  `EqualityBasedCondition`, which you name to type an assignment list built at
  run time; it is covered by the usual API-level rules.
