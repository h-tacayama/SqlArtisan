...
The packages expose two namespaces, and they carry different promises.
...
- **`SqlArtisan.Internal`** — the values the API hands back. A `Sql.*` call
  ...
  **Not covered**: constructing one with `new`, deriving from one, and any
  member the reference does not document. Every one of these types is reached
  through a `Sql.*` call; there is no supported path that starts with `new`.

The split is gated, not merely stated: a public type in `SqlArtisan.Internal`
that no public signature hands back fails the test suite, so the namespace
cannot quietly accumulate surface the commitment above was never meant to
cover.
