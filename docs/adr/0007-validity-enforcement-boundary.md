...
that some engine happens not to support (e.g. `CUBE` on MySQL, `DISTINCT ON` on
SQL Server). That is emitted faithfully (ADR 0001) and surfaced by the opt-in
analyzer (ADR 0003), with the database as the final arbiter. The library never
throws for it.
