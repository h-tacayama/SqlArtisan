...
- **A new ID, one ID for all context rules.** `SQLA0001`'s message embeds
  ...
  warnings. One shared ID (not one per rule) matches how `SQLA0001` covers
  ~230 constructs and `SQLA0003` covers many positions; per-rule severity is
...
- **Suppression is standard Roslyn only** (`#pragma`, `[SuppressMessage]`,
  ...
  ships now — `ValidateConfiguration` (SQLA0002) is untouched. If MySQL ever
...
- **Reusing `SQLA0001`.** Needs a second message format under one ID and
...
- A query can legitimately draw both warnings — `GroupBy(Rollup(x))` on
  MySQL fires `SQLA0001` for `Rollup` and `SQLA0004` for a `Grouping` in the
...
- The trigger set is deliberately minimal: `Limit` alone covers the
  ...
  so `SQLA0001` fires regardless of position).
