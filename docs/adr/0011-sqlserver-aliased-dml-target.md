...
The mechanism is a `ValidateForDialect(Dbms)` hook on `SqlBuilderBase`, invoked
at the top of `BuildCore`, that `UpdateBuilder` and `DeleteBuilder` override —
so every build path (including `Returning()`, which funnels through `BuildCore`)
is covered.
