...
- **The legacy pair still works, desugars exactly as before, and now also
  ...
  real config-error detection. `SQLA0002` fires whenever a resolvable legacy pair
  governs the resolution (no family key present; an unrecognized value is
  `SQLA0001`'s alone) — even when the pair
