...
> [!NOTE]
> `Position()` and `Strpos()` both find a substring's 1-based index, but MySQL
> and PostgreSQL spell it as the keyword form `POSITION(substr IN str)`, while
> PostgreSQL alone also accepts the function-call form `STRPOS(str, substr)` —
> note the reversed argument order. Neither runs on Oracle, SQLite, or SQL
> Server, which spell this `INSTR`/`CHARINDEX` instead.
