...
```csharp
SqlStatement sql =
    Select(u.CreatedAt - IntervalLiteral("30", DateTimePart.Day))
    .From(u)
    .Build(Dbms.Oracle);
...
- **PostgreSQL** — the same two forms as Oracle, plus the bare
  `IntervalLiteral(text)` overload with the unit folded into the text
  (`INTERVAL '30 days'`).
