...
| Oracle | ✕(VALUES 不可) | 配列バインド | **別パッケージ** | VALUES が無いため配列バインドで高速化 |
...
- **Oracle の実装方式 = 配列バインド(`OracleCommand.ArrayBindCount`)で確定。**
  全 ODP.NET で確実に使え、bulk-copy 匹敵速度、ドライバ依存リスクが低い。
  `OracleBulkCopy` は ODP.NET ドライバ種別依存のため不採用(将来の追加最適化としてのみ余地)。
