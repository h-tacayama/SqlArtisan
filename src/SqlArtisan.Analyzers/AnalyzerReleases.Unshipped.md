...
SQLA0104 | SqlArtisan.Dialect | Warning | A literal argument value the target dialect rejects: a DateTimePart outside what it accepts for that function, a RegexpOptions member outside its match-parameter alphabet, a negative constant row count, or a GROUP BY column ordinal on an engine that has no select-list positions.
