...
public abstract class SqlPart
{
    ...
    // ConditionGuard, to reject a written clause whose whole condition is empty
    // (the #236 empty-state policy). Default false; the condition types override
    // it. The check is recursive — an AND/OR/NOT tree whose operands are all empty
    // is itself empty.
