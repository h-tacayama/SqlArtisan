...
{
    /// <summary>Prepends <c>WITH cte AS (...), ...</c> before the feeding <c>SELECT</c>.</summary>
    ...
    /// <summary>Prepends <c>WITH RECURSIVE cte AS (...), ...</c> before the feeding <c>SELECT</c>, allowing a CTE to reference itself.</summary>
    /// <param name="ctes">One or more CTE definitions, each produced by <c>cte.As(subquery)</c>.</param>
    /// <returns>The builder positioned to write the <c>SELECT</c> that draws on the CTEs.</returns>
