...
// The Build()-time guard shared by UpdateBuilder and DeleteBuilder for an
// aliased UPDATE/DELETE target on SQL Server. T-SQL cannot alias the target
// directly (the alias must come from a FROM clause — the joined-DML form), so
// the text SqlArtisan would emit, `UPDATE users AS "cu" ...`, is a syntax error
// there. The exact text is valid on PostgreSQL/MySQL, which by ADR 0007 makes it
// dialect availability (normally permissive) — but the alias is a value-level
