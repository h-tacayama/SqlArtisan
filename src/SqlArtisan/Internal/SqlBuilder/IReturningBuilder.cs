...
/// <summary>
/// The state after <c>RETURNING</c>: build directly, or bind the returned expressions to Oracle
/// PL/SQL output variables with <c>INTO</c>.
/// </summary>
public interface IReturningBuilder : ISqlBuilder
{
    ...
    /// <exception cref="ArgumentException">No output parameters were supplied, their count does not match the number of <c>RETURNING</c> expressions, or a returned expression carries an <c>.As(...)</c> alias (the output parameter names the value).</exception>
