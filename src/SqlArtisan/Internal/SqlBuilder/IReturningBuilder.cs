...
/// <summary>The state after <c>RETURNING</c>: build directly, or bind the returned expressions to Oracle PL/SQL output variables with <c>INTO</c>.</summary>
public interface IReturningBuilder : ISqlBuilder
{
    /// <summary>Appends <c>INTO :var, ...</c>, binding each returned expression to a named output variable (Oracle PL/SQL <c>RETURNING ... INTO</c>).</summary>
