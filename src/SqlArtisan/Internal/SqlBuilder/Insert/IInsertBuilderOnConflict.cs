...
{
    /// <summary>
    /// Appends <c>DO NOTHING</c>, silently skipping the row on a conflict.
    /// </summary>
    /// <returns>The builder positioned for <c>RETURNING</c> or build.</returns>
    /// <remarks>PostgreSQL and SQLite syntax.</remarks>
    IReturning DoNothing();

    /// <summary>
    /// Appends <c>DO UPDATE SET ...</c>, updating the conflicting row; reference the proposed row
    /// with <see cref="ExcludedColumn"/> (<c>EXCLUDED.col</c> on PostgreSQL, <c>excluded.col</c> on
    /// SQLite).
    /// </summary>
    ...
    IInsertBuilderDoUpdateSet DoUpdateSet(params EqualityCondition[] assignments);
}
