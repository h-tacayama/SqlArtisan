...
/// <summary>The state after <c>DELETE FROM table</c>: narrow with <c>WHERE</c>, add <c>RETURNING</c>, or build.</summary>
public interface IDeleteBuilderDelete : ISqlBuilder, IReturning
{
    /// <summary>Appends <c>WHERE condition</c> to restrict which rows are deleted.</summary>
