...
/// <summary>
/// The state after a <c>JOIN</c> inside an <c>UPDATE ... FROM</c>: supply the
/// join predicate with <c>On(...)</c> or <c>Using(...)</c>.
/// </summary>
public interface IUpdateBuilderFromJoinOn
{
    /// <summary>
    ...
    IUpdateBuilderFrom On(SqlCondition condition);
    ...
    /// <returns>The builder back in the <c>FROM</c> state, ready for further joins, <c>WHERE</c>, <c>RETURNING</c>, or build.</returns>
    IUpdateBuilderFrom Using(DbColumn column, params DbColumn[] additionalColumns);
}
