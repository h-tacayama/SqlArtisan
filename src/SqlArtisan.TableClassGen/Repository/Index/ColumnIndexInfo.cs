...
// Two facts per table: which columns lead a plain index, and which are named by
// some index expression. The second only ever produces silence — an expression
// index exists precisely so the wrapped predicate can be written.
    ...
    bool allUnknown = false)
{
    ...
    // which the emitter writes as an absent argument.
    public bool? IsIndexed(string columnName) =>
        allUnknown || MentionedByExpression(columnName)
            ? null
            : leadingColumns.Contains(columnName, StringComparer.Ordinal);
