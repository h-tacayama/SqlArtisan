...
## [Unreleased]
### Added
- Added support for string aggregation, exposed per dialect (no unified rewrite): `StringAgg()` (PostgreSQL/SQL Server), `Listagg()` (Oracle), and `GroupConcat()` (MySQL/SQLite). Each emits its dialect-native syntax verbatim, and ordering follows the dialect: PostgreSQL takes an inline `.OrderBy(...)`, SQL Server and Oracle take `.WithinGroup(OrderBy(...))`. For `GROUP_CONCAT`, SQLite's positional separator is `GroupConcat(expr, sep)` (a bind parameter) while MySQL's keyword form is `GroupConcat(expr, Sql.Separator(sep))`, which also accepts `.OrderBy(...)` and a `Distinct` overload; MySQL requires the `SEPARATOR` value to be a string literal, so it is emitted inline as an escaped literal rather than a bind parameter. Note: MySQL silently truncates `GROUP_CONCAT` output at `group_concat_max_len` (1024 bytes by default); raise that variable for large groups. (#88)
