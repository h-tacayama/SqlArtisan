...
## [Unreleased]
### Added
- Added collection overloads to `Values(...)` for a collection-driven multi-row `INSERT` — `InsertInto(t, cols...).Values(rows)` emits one `VALUES (...)` row per element of a `List<object[]>`, an `object[][]` (e.g. `.Select(...).ToArray()`), or any `IEnumerable<object[]>`, instead of a `Values(...)` call per row. The output is identical to the chained form (one bind per value). Per-row width validation reuses the existing multi-row guard (a mismatched row throws, naming it), and an empty row collection throws at the call site (`VALUES requires at least one row; the row collection is empty.`) rather than emit an invalid empty `VALUES`. A companion `object[][]` overload keeps a jagged array from being ambiguous with the `params object[]` single-row form. Available on the with-columns and no-columns entry states, and their `INSERT IGNORE` siblings. See the [Multiple Rows](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/query-statements.md#multiple-rows) section. (#243)
- Added collection overloads to `In` / `NotIn` — `col.In(ids)` binds a `List<T>`, `T[]`, `HashSet<T>`, or any `IReadOnlyCollection<T>` directly, one bind per element, without the prior `.Cast<object>().ToArray()` ceremony. The parameter is `IReadOnlyCollection<T>`, deliberately not `IEnumerable<T>`: a `string` is an `IEnumerable<char>` but not an `IReadOnlyCollection<char>`, so `col.In("abc")` stays a single-value predicate (`IN (:0)`) rather than silently expanding into one bind per character. A companion typed-array overload keeps a reference-type array (`string[]`, covariantly convertible to `object[]`) from becoming ambiguous with the existing `params object[]` form — it binds one value per element either way, so existing array callers are unaffected. An empty collection throws at the call site (`IN requires at least one value.`). Each element is a separate bind, so watch per-engine parameter ceilings on very large lists — see the [IN Condition](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/expressions.md#in-condition) section. (#243)
- Added `NoCondition`, the neutral `SqlCondition` — the identity element for both `&` and `|` — for folding a variable-length list of predicates without a dummy seed: `filters.Aggregate(NoCondition, (a, b) => a & b)`. Every real condition combined onto it survives and drops the seed out cleanly, so an `n`-element list yields exactly `n` predicates, and the same seed folds an `AND` list and an `OR` list identically (it is *absence*, not `TRUE` — a `TRUE` seed would absorb an `OR` fold). It carries no new build-time behavior: a `WHERE` that reduces to nothing but `NoCondition` follows the existing empty-clause policy and throws at `Build()` (#236) — omit `Where(...)` to run unfiltered on purpose. See the [Dynamic Condition](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/expressions.md#dynamic-condition) section. (#243)
- Added task-oriented documentation (#227): two guides — [Dapper + SqlArtisan from scratch](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/guides/dapper-quickstart.md) and [using SqlArtisan with AI coding assistants](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/guides/ai-assistants.md) — plus a [query cookbook](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/cookbook.md) of realistic end-to-end queries whose emitted SQL is pinned by exact-SQL unit tests.
- Added `Asterisk` for the bare `*` select item (`Select(Asterisk)` → `SELECT *`) and a `.Asterisk` member on every table class, CTE, and derived table for the qualified star (`Select(u.Asterisk)` → `SELECT "u".*`, or `users.*` when the table has no alias) — every column of one relation, composable beside other select items in a join. Both also work in `RETURNING` (`Returning(Asterisk)` → `RETURNING *`) and are select-item-only — an expression position (`Upper(...)`, `ORDER BY`, …) rejects them, except `Count(Asterisk)` (`*` is legal in no other aggregate). Universal syntax, with one mixing rule: on Oracle a bare `*` must be the only select item — beside other items, use the qualified star. Note the pre-existing trap it resolves: `Select("*")` binds the string and emits `SELECT :0` (a string is always a value, never SQL). See `docs/query-statements.md`. (#242)
...
- Added a `Count(Asterisk)` overload emitting `COUNT(*)` (row count, `NULL`s included) alongside the existing `Count(expr)` (non-`NULL` count). Composes with `.Filter(...)` and `.Over(...)` like any other aggregate. Identical in syntax and semantics on every dialect. Originally landed as a parameterless `Count()`; respelled to `Count(Asterisk)` before release once the `Asterisk` marker (#242) made the SQL-visible form expressible. (#233)
...
### Fixed
- `Trim(source, trimChar)`'s dialect note now spells out *why* SQLite rejects it: the 2-argument form emits the ANSI `TRIM(BOTH trimChar FROM source)`, and SQLite's grammar has no `BOTH ... FROM` clause — distinct from SQLite's own positional `trim(source, trimChar)`, which SqlArtisan does not emit. Verified against live MySQL, PostgreSQL, SQLite, and SQL Server. Doc-only; emitted SQL is unchanged. (#296)
- `PercentileCont(fraction)` / `PercentileDisc(fraction)` now reject a fraction outside the `0..1` domain at the call site, instead of building valid-looking SQL (`PERCENTILE_CONT(1.5) ...`) every engine rejects at execution time. The factories already refused a non-finite fraction; the range check extends that same eager guard (`The percentile fraction must be in the range 0 to 1.`). The bounds are inclusive — `0` and `1` still build. (#295)
- `Hints("")` / `Hints(null)` no longer emit a stray double space after `SELECT` (`SELECT  col` → `SELECT col`); a non-empty hint is unchanged. Bundled with a doc/style tidy-up of the public `Sql.*` surface (deduplicated `DateTimePart`/`GroupConcat` remarks, restored alphabetical order in `Sql.M.cs`, trimmed over-length `StringAgg`/`Rollup` remarks). (#294)
- `Rollup`'s dialect note now names SQLite alongside MySQL as unsupported, matching the matrix, and the `GroupConcat` matrix entry documents its known overload collision like its siblings. Doc-only; emitted SQL is unchanged. (#293)
- Corrected two XML doc claims that contradicted actual behavior: `InsertInto(table)` / `InsertIgnoreInto(table)` (the column-less overloads) no longer promise a `.Select(...)` continuation their return type doesn't expose, and `ToNumber(expr)`'s 1-argument form now reads "Oracle syntax" instead of "Oracle and PostgreSQL" (PostgreSQL's `to_number` requires the format argument). Also filled in missing dialect notes on several sibling string/lock functions. Doc-only; emitted SQL is unchanged. (#292)
- `Cast(expr, type)`, `JsonExtract` / `JsonQuery` / `JsonValue`, `GroupingSets(...)`, and `Currval(name)` now reject a missing required argument at the call site instead of emitting invalid SQL (`CAST(id AS )`, `CURRVAL('')`) or throwing a bare `NullReferenceException` at `Build()`. Each throws naming the construct (e.g. `CAST requires a target type.`). (#291)
- Empty params-collection factories now fail loudly instead of emitting structurally invalid SQL, extending the eager empty-collection guard (already on `Select`, `PartitionBy`, `OrderBy`, `DistinctOn`, `GroupBy`) to the remaining sites: `Case(...)` with zero `WHEN` clauses (previously `CASE id  END`), `Least()` / `Greatest()` with no arguments (`LEAST()` / `GREATEST()`), and `With()` / `WithRecursive()` with no CTEs (bare `WITH `). Each throws `ArgumentException` at the call site naming the construct (`CASE requires at least one WHEN clause.`, `LEAST requires at least one expression.`, `WITH requires at least one common table expression.`). A sweep of the other `params`-collection factories found no further offenders — `InsertInto` with zero columns already degrades to the valid no-column form, `Group()`'s empty grouping set is valid, and the rest require a leading argument. (#290)
- `Nextval(name)` and `Currval(name)` now escape the sequence name as a proper string literal — doubling `'` on every dialect and `\` on MySQL — instead of splicing it in raw. A name containing a single quote previously broke out of its quotes (e.g. `NEXTVAL('seq'; DROP TABLE users; --')`); it now emits the escaped `NEXTVAL('seq''; DROP TABLE users; --')`, matching every other literal-emitting path. (#289)
