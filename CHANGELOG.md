...
- **Breaking:** removed the `DbTableBase(string tableAlias)` constructor, whose single string was the *alias* (the table name silently defaulted to the subclass's type name) while the sibling bases' single string is the *name* — a wrong-table trap for hand-written table classes. Use `DbTableBase(tableName, tableAlias)`; pass an empty alias for none. (#149)
