...
## [Unreleased]
### Added
- Added `SQLA0006` and the `sqlartisan_target_version` (`<SqlArtisanTargetVersion>`) config key it reads: a build-time warning when a construct is supported on the target dialect but not until a *newer* engine version than the one you've declared — e.g. `MERGE` before PostgreSQL 15, `DATETRUNC` before SQL Server 2022. The matrix now carries an optional minimum-version floor per construct/dialect, sourced only from evidence already on record; declaring no version (or one that meets every bound) reproduces today's behavior exactly, and `sqlartisan_construct_*` overrides still keep the last word. A pinned `gvenzl/oracle-free:23-slim-faststart` integration lane live-proves the Oracle bounds against a real 23ai engine — including `WITH RECURSIVE` (Oracle 23ai+), whose live-proof round-trip caught and fixed a real emitted-SQL gap; see the Fixed entry below. See [Version-aware warnings](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#version-aware-warnings-sqla0006). (#263)
...
### Changed
- **Breaking:** renumbered the analyzer diagnostic IDs into semantic order — configuration first, then what the construct is, where it sits, and how it's named: `SQLA0001` is now the invalid-configuration warning (previously `SQLA0002`) and `SQLA0002` the dialect-mismatch warning (previously `SQLA0001`); the two rules new in this release slot in as `SQLA0003` (context rules) and `SQLA0004` (identifier length). A `dotnet_diagnostic.SQLA000x.severity` line, `#pragma warning disable`, or `[SuppressMessage]` written against 0.6.0-beta.1 now targets the other diagnostic — swap `SQLA0001` and `SQLA0002` when upgrading. What each rule reports is unchanged; see the [rules table](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#rules). (#264)
...
### Fixed
- `WithRecursive(...)`'s CTE body now emits an explicit `AS` for a `.As(...)`-aliased column (`"a".code AS cte_code`, not `"a".code cte_code`) — Oracle 23ai's `RECURSIVE` clause rejects the bare form with `ORA-02000: missing AS keyword`, even though the identical shape is valid everywhere else, including Oracle's own plain `With(...)`. Emitted SQL for `WithRecursive(...)` column aliases changes accordingly; `With(...)` and every other position `.As(...)` formats in are unaffected. Caught by the pinned Oracle 23ai integration lane (#263).
