...
### Fixed
- Empty-condition states no longer emit invalid SQL. When every condition is excluded (e.g. all `ConditionIf(false, …)`), a `SELECT`'s `WHERE` / `HAVING` and an aggregate's `FILTER (WHERE …)` are now elided instead of emitting a bare `WHERE` / `HAVING` / `FILTER ()`, and a nested all-empty `AND` / `OR` / `NOT` subtree renders nothing rather than a stray `()` — even when an active condition sits beside it. Positions where "no condition" is not a plausible intent fail loudly at `Build()` instead: an all-empty `WHERE` on an `UPDATE` or `DELETE` (eliding it would silently widen a filtered statement to a full-table write — omit `.Where(...)` for an intentional one), a `JOIN` / `MERGE` `ON`, a `CASE` `WHEN`, and a `MERGE` `WHEN MATCHED` / `DELETE WHERE` condition. An empty `SELECT` list (`Select()`) now throws eagerly. (#236)
