...
## [Unreleased]
### Added
- Added support for joining a correlated derived table via `APPLY` / `LATERAL`: `CrossApply(subquery, schema)` / `OuterApply(subquery, schema)` (`CROSS APPLY` / `OUTER APPLY`, SQL Server / Oracle), `CrossJoinLateral(subquery, schema)` / `LeftJoinLateral(subquery, schema)` (`CROSS JOIN LATERAL` / `LEFT JOIN LATERAL ... ON true`, PostgreSQL / MySQL / Oracle), and `JoinLateral(subquery, schema).On(cond)` (`JOIN LATERAL ... ON ...`). The derived table is named by an `AdHocDerivedTable` handle (referenced through it rather than via a repeated alias string, with ad-hoc `Column(name)` access, and emitted bare to match CTE naming). A matching `AdHocCte` lets a one-off CTE skip declaring a `CteSchemaBase` subclass (use `Column(name)` for its columns). `APPLY` and `LATERAL` are distinct grammars for the same intent, so each is exposed as its own method per ADR 0002 — `Build(Dbms)` emits faithfully and never rewrites one form into the other; availability (SQLite supports neither; `LATERAL` has no SQL Server form) is left to the database and the analyzer (ADR 0003). (#122)
