...
- **Breaking:** the types ordinary application code must *name* — a helper method's return type, an accumulator variable, a `List<>` element, a shared `static readonly` field — moved from `SqlArtisan.Internal` to the root `SqlArtisan` namespace: `SqlCondition`, `SqlExpression`, `ISubquery`, `TableReference`, `SortOrder`, `ExpressionAlias`, `CommonTableExpression`, and `DbSequence`. Previously the standard dynamic-query idioms (a nullable condition accumulator, a reusable-subquery helper, a dynamic `FROM` / `WITH` list, a UI-driven sort key, a shared sequence handle) forced an implementation-detail `using SqlArtisan.Internal;` just to name them. The line is a query's **content** — the relations, values, predicates, and sort items that determine the result set — versus **clause syntax and statement decoration**: clause fragments (`OrderBy(...)` for `OVER` / string aggregation, `Of(...)`, `Separator(...)`), optimizer hints, and lock behaviors stay in `Internal`, specified through the builder chain, and the reusable unit there is the completed expression, which the moved roots already name. Every concrete node (`AndCondition`, `LikeCondition`, `CountFunction`, …) stays in `Internal` and is held only through these roots; code that named the moved types fully qualified needs a one-line fix, and a now-unnecessary `using SqlArtisan.Internal;` is harmless. `CommonTableExpression`'s constructor is now `internal` — obtain it via `Cte` / `CteBase`'s `.As(...)`, the path that also yields the column-reference handle. (#244)
### Fixed
