...
## [Unreleased]
### Added
- Published the project's trust baseline: `SECURITY.md` (private vulnerability reporting via GitHub Security Advisories, what counts as a vulnerability for a query builder, supported versions, best-effort response — no guaranteed SLA on an individually-maintained project) and a versioning & support policy (`docs/versioning.md`) — the SemVer commitment from 1.0, what counts as breaking (a change to the emitted SQL for the same input is at minimum a *minor* release, called out under a dedicated **Emitted SQL** CHANGELOG heading; builder-stage `I*Builder*` interfaces are not for user implementation, so adding members to them is *minor*), the deprecation process (`[Obsolete]` in a minor release, removal no earlier than the next major), supported runtimes, verified engine versions, and release cadence. Linked from the README. (#224)
