...
## [Unreleased]
### Added
- Added support for joining a correlated derived table via `APPLY` / `LATERAL`: `CrossApply(subquery, alias)` / `OuterApply(subquery, alias)` (`CROSS APPLY` / `OUTER APPLY`, SQL Server / Oracle), `CrossJoinLateral(subquery, alias)` / `LeftJoinLateral(subquery, alias)` (`CROSS JOIN LATERAL` / `LEFT JOIN LATERAL ... ON true`, PostgreSQL / MySQL / Oracle), and `JoinLateral(subquery, alias).On(cond)` (`JOIN LATERAL ... ON ...`). `APPLY` and `LATERAL` are distinct grammars for the same intent, so each is exposed as its own method per ADR 0002 — `Build(Dbms)` emits faithfully and never rewrites one form into the other; availability (SQLite supports neither; `LATERAL` has no SQL Server form) is left to the database and the analyzer (ADR 0003). (#122)
