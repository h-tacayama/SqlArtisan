...
### Changed
- **Breaking:** a row-limited query no longer loses its subquery capability — the builder states reached through `Limit()` / `Offset()` / `OffsetRows()` / `FetchNext()` / `FetchFirst()` can now be embedded like any other subquery: as a `LATERAL` / `APPLY` operand (the canonical per-group top-N), an aliased scalar select item (`.As("alias")`), a CTE body, and an `IN` / `NOT IN` / `EXISTS` / `ALL` / `ANY` / `SOME` operand. The change is source-compatible for callers but binary-breaking: the terminal fluent steps (`Offset()` in both families, `FetchFirst()`, `FetchNext()`) now return the new `ISelectBuilderPaginated` instead of `ISqlBuilder`, so code compiled against an earlier version must be rebuilt. Recompiling also changes one behavior by design: `In(...)` / `NotIn(...)` with a row-limited subquery previously fell through to the scalar-subquery fallback and silently emitted a double-parenthesized scalar comparison — `IN ((SELECT … LIMIT :0))`, wrong semantics that fail at runtime as soon as the subquery returns a second row — and now resolves to the subquery overload and emits the list form `IN (SELECT … LIMIT :0)`. Note that MySQL itself rejects `LIMIT` directly inside an `IN` / `ALL` / `ANY` / `SOME` subquery; route the limited query through a CTE there (see the pagination reference). (#240)
### Fixed
