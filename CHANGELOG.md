...
## [Unreleased]
### Added
- Added support for collection-driven bulk `INSERT` via `BuildBatches()` on the multi-row `INSERT ... VALUES` builder. It splits the rows into the fewest statements that each stay under the target dialect's bind-parameter cap (SQL Server 2100, SQLite 999, PostgreSQL/MySQL 65535), returning an `IReadOnlyList<SqlStatement>` to execute in turn. Each batch carries any `ON CONFLICT ...` / `ON DUPLICATE KEY UPDATE` clause, so multi-row UPSERT works unchanged. Oracle has no multi-row `VALUES` and throws `NotSupportedException`. (#87)
