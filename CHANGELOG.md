...
## [Unreleased]
### Added
- Added the `SqlArtisan.BulkCopy` package — Oracle high-throughput bulk INSERT via ODP.NET array binding (`OracleCommand.ArrayBindCount`): `connection.BulkInsert(table, rows)` / `BulkInsertAsync(...)` map a table class's columns to same-named row-object properties, send every row in one round trip, and fail loudly on an empty row set, a missing row property, or an unmapped property type. See the [Oracle bulk insert guide](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/guides/bulkcopy-oracle.md). (#90)
- Added `DbTableBase.TableName`, a public read of the table's SQL identifier as declared (e.g. `"users"`) — the table-level counterpart to `DbColumn.Name`, completing the public surface a satellite package needs to compose per-table SQL from a table class. (#90)
- Added `DbColumn.Name`, a public read of a column's SQL identifier as declared (e.g. `"created_at"`) — lets an integration or tooling package resolve a table class's columns by reflecting over its public `DbColumn` properties, without reaching into internals. (#90)
