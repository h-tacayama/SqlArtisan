...
### Tests
- Added a real-database integration test matrix (`tests/SqlArtisan.IntegrationTests`) that executes SqlArtisan-built statements against all five engines — MySQL, Oracle, PostgreSQL, SQL Server (via Testcontainers) and SQLite (in-process) — catching grammar and semantic bugs only the engine itself rejects. Coverage spans the query surface (`SELECT`; `INNER`/`LEFT`/`RIGHT`/`FULL`/`CROSS JOIN`; CTE and recursive CTE; window functions and frames; `GROUP BY` with its `ROLLUP`/`CUBE`/`GROUPING SETS` extensions and multi-column grouping; aggregates including `COUNT(DISTINCT)`; the set operators `UNION`/`UNION ALL`/`EXCEPT`/`INTERSECT`/`INTERSECT ALL`; `DISTINCT`; the `WHERE` predicates `BETWEEN`/`LIKE`/`IS NULL`/`IN`/`NOT` and their negations; ordering including `DESC` and `NULLS FIRST`/`LAST`; `APPLY`/`JOIN LATERAL`), DML (`INSERT`/`UPDATE`/`DELETE`, expression-valued `SET`, per-dialect pagination, UPSERT, `MERGE` with all `WHEN` branches, `RETURNING` and Oracle `RETURNING … INTO`), the Dapper integration (sync and async entry points, the ad-hoc `DbTable` reference, typed parameter binding and value round-trips), and edge cases (`NULL`, Unicode, special characters, `decimal`/`DateTime`/`boolean`). The exact-SQL unit tests remain the fast inner loop; the integration matrix runs nightly, on demand, and gates releases. (#151)
