...
- Added `SQLA0011`, a build-time warning for a `WHERE` or `ON` predicate that wraps an indexed column in a function, or matches it with a leading-wildcard pattern — either shape keeps the index from being used. See [Schema-aware warnings](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#schema-aware-warnings-sqla0007). (#266)
...
- Added `SQLA0009`, a build-time warning for an `INSERT` whose column list omits a column the generated table class declares `NOT NULL` with no default — a row the engine rejects. A column the engine assigns itself (identity, auto-increment, generated, or one with a `DEFAULT`) is never reported. See [Schema-aware warnings](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#schema-aware-warnings-sqla0007). (#266)
