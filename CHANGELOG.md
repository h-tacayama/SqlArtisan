...
## [Unreleased]
### Added
- Added support for joining a correlated derived table via `APPLY` / `LATERAL`: `CrossApply(subquery, schema)` / `OuterApply(subquery, schema)` (`CROSS APPLY` / `OUTER APPLY`, SQL Server / Oracle), `CrossJoinLateral(subquery, schema)` / `LeftJoinLateral(subquery, schema)` (`CROSS JOIN LATERAL` / `LEFT JOIN LATERAL ... ON true`, PostgreSQL / MySQL / Oracle), and `JoinLateral(subquery, schema).On(cond)` (`JOIN LATERAL ... ON ...`). The derived table is named by a `DerivedTableSchemaBase` handle (referenced through it rather than via a repeated alias string, and emitted bare to match CTE naming): use the general-purpose `DerivedTable` with ad-hoc `Column(name)` access, or subclass `DerivedTableSchemaBase` to expose typed `DbColumn` members. CTEs share the same model — `CteSchemaBase` now derives from `DerivedTableSchemaBase`, and a new general-purpose `Cte` (with `Column(name)`) lets a one-off CTE skip declaring a schema class. `APPLY` and `LATERAL` are distinct grammars for the same intent, so each is exposed as its own method per ADR 0002 — `Build(Dbms)` emits faithfully and never rewrites one form into the other; availability (SQLite supports neither; `LATERAL` has no SQL Server form) is left to the database and the analyzer (ADR 0003). (#122)
