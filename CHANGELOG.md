...
## [Unreleased]
### Added
- Added `SQLA0003`, a build-time warning when a compile-time identifier literal — a table or expression alias, a CTE or derived-table name, a `VALUES` column name, or an Oracle `RETURNING ... INTO` output variable — is longer than the target dialect allows (MySQL 64 characters, Oracle 128 bytes, PostgreSQL 63 bytes, SQL Server 128 characters; SQLite is unbounded). PostgreSQL silently truncates an over-long identifier rather than erroring, so the analyzer is the only place it surfaces before the database; Oracle and PostgreSQL measure in UTF-8 bytes. Only constant identifiers are checked, and any site is suppressible the standard Roslyn way. See the [analyzer guide](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#rules). (#265)
