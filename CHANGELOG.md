...
## [Unreleased]
### Added
- Added support for string aggregation, exposed per dialect (no unified rewrite): `StringAgg()` (PostgreSQL/SQL Server), `Listagg()` (Oracle), and `GroupConcat()` (MySQL/SQLite), each emitted verbatim so the caller picks the form their target DBMS requires. Ordering follows the dialect, via an inline `OrderBy(...)` argument or a chained `.WithinGroup(OrderBy(...))`. Note: MySQL requires the `GROUP_CONCAT` separator to be a literal (emitted via `Sql.Separator(...)`) and silently truncates the result at `group_concat_max_len` (1024 bytes by default). (#88)
