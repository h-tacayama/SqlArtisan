...
### Fixed
- `Trim(source, trimChar)`'s dialect note now spells out *why* SQLite rejects it: the 2-argument form emits the ANSI `TRIM(BOTH trimChar FROM source)`, which SQLite's grammar has no `BOTH ... FROM` clause for — distinct from SQLite's own positional `trim(source, trimChar)`, which SqlArtisan does not emit. Verified against live SQLite/MySQL/PostgreSQL/SQL Server. Doc-only; emitted SQL is unchanged. (#296)
