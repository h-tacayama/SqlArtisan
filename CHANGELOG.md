...
### Changed
- **Breaking:** an incomplete statement no longer compiles where it previously built invalid SQL: a compound query ending in a set operator (`... .Union` with no following `SELECT`), an `UPDATE` without `SET`, an `INSERT` / `INSERT IGNORE` without a row source, and a `MERGE` with no `WHEN` branch all lose their `Build(...)` — the builder stages for those states no longer extend `ISqlBuilder`, so the misuse is a compile error instead of a statement no dialect accepts. Complete chains are unaffected and their emitted SQL is unchanged. (#149)
