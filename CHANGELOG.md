...
### Fixed
- Corrected two XML doc claims that contradicted actual behavior and filled in the dialect notes several sibling string functions were missing. `InsertInto(table)` / `InsertIgnoreInto(table)` (the column-less overloads) no longer promise a `.Select(...)` continuation their return type doesn't expose — only the column-list overloads support it. `ToNumber(expr)`'s 1-argument form now reads "Oracle syntax" (not "Oracle and PostgreSQL") to match the matrix, since PostgreSQL's `to_number` requires the format argument. Added matrix-matching dialect notes to `Of`, `Nowait`, the 2-argument `Trim` / `Rtrim` / `Ltrim`, `Substr`, and the padding-argument `Rpad` / `Lpad` overloads, and a doc/matrix parity test guarding against this drift. Doc-only; emitted SQL is unchanged. (#292)
