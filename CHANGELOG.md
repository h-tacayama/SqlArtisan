...
## [Unreleased]
### Added
- Added `Value(x)`, a bind-parameter expression for a literal value, enabling aliasing and operator composition that bare literals cannot do — `Value(1).As("depth")` emits `:0 "depth"`, `Value(100) - u.Age` emits `(:0 - age)`. The emission is always a bind parameter; when an explicit SQL type is needed (e.g. a recursive-CTE anchor), use `Cast(...)` instead. See `docs/expressions.md`. (#243 ERG-02)
