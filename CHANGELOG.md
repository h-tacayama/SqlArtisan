...
- **Breaking:** the types ordinary application code must *name* — a helper method's return type, a `WHERE` / sort-key accumulator variable, a `List<>` of tables or CTEs — moved from `SqlArtisan.Internal` to the root `SqlArtisan` namespace: `SqlCondition`, `SqlExpression`, `ISubquery`, `TableReference`, `SortOrder`, `ExpressionAlias`, and `CommonTableExpression`. Previously the standard dynamic-query idioms (a nullable condition accumulator, a reusable-subquery helper, a dynamic `FROM` / `WITH` list, a UI-driven sort key) forced an implementation-detail `using SqlArtisan.Internal;` just to name them. Every concrete node (`AndCondition`, `LikeCondition`, `CountFunction`, …) stays in `Internal` and is held only through these roots; code that named the moved types fully qualified needs a one-line fix, and a now-unnecessary `using SqlArtisan.Internal;` is harmless. `CommonTableExpression`'s constructor is now `internal` — obtain it via `Cte` / `CteBase`'s `.As(...)`, the path that also yields the column-reference handle. (#244)
### Fixed
