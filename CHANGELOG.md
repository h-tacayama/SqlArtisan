...
### Fixed
- Closed the last dialect-parity gaps flagged in the public-API review (#293): `Rollup`'s `<remarks>` now names SQLite alongside MySQL as lacking the function form (the matrix marks both unsupported), and the `GroupConcat` matrix entry gained the collision caveat comment its siblings (`Match`, `Concat`, `Instr`, `Using`) already carry — its three arity-2 overloads collapse to one MySQL/SQLite union key, so a MySQL-only form used on SQLite stays silent (under-restricts, never a false positive). Doc/comment-only; emitted SQL and the matrix values are unchanged. (#293)
