...
### Fixed
- Empty-condition states now fail loudly instead of emitting invalid SQL. A written condition clause whose every operand is excluded (e.g. all `ConditionIf(false, …)`) is rejected at `Build()` rather than silently dropped — the library never elides a clause you wrote. This is uniform across `WHERE` (on `SELECT`, `UPDATE`, and `DELETE` alike — an unintended `WHERE`-less read or write is a real hazard), `HAVING`, an aggregate's `FILTER (WHERE …)`, a `JOIN` / `MERGE` `ON`, a `CASE` `WHEN`, and a `MERGE` `WHEN [NOT] MATCHED [BY SOURCE] AND` / `DELETE WHERE` condition, each with a message naming the clause. To run without a restriction, omit the clause entirely — the explicit, unambiguous way to say "no filter". An excluded operand *beside* an active one still just drops out (so `ConditionIf` dynamic search keeps working), and a nested all-empty `AND` / `OR` / `NOT` subtree contributes nothing rather than a stray `()`. An empty `SELECT` list (`Select()`) throws eagerly. (#236)
