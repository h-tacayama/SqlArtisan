...
- Added collection overloads to `Values(...)` and `In` / `NotIn` — pass a `List<T>`, `T[]`, `HashSet<T>`, or any `IReadOnlyCollection<T>` directly instead of spreading elements by hand. `Values(rows)` accepts `List<object[]>`, `object[][]`, or `IEnumerable<object[]>` for multi-row `INSERT`. An empty collection throws at the call site. See the [Multiple Rows](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/query-statements.md#multiple-rows) and [IN Condition](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/expressions.md#in-condition) sections. (#243)
...
- Added `InsertIgnoreInto(table)` for MySQL's `INSERT IGNORE`. The builder chain omits UPSERT clauses — `.OnDuplicateKeyUpdate(...)` / `.OnConflict(...)` do not compile after it. On PostgreSQL/SQLite, use `OnConflict().DoNothing()` instead. See `docs/query-statements.md`. (#238)
...
- Added `Grouping(expr)` (MySQL 8.0.1+, Oracle, PostgreSQL, SQL Server) and `GroupingId(a, ...)` (Oracle, SQL Server) for `Rollup` / `Cube` / `GroupingSets` subtotal-row labeling. (#235)
...
- **Breaking:** a query with a row-limiting clause (`Limit` / `Offset` / `OffsetRows` / `FetchNext` / `FetchFirst`) can now be used as a subquery — a `LATERAL` / `APPLY` operand, an aliased scalar select item, a CTE body, or an `IN` / `EXISTS` / `ALL` / `ANY` / `SOME` operand. Binary-breaking — `Offset()` / `FetchFirst()` / `FetchNext()` now return `ISelectBuilderPaginated` instead of `ISqlBuilder` — rebuild against this version. (#240)
- **Breaking:** `SqlCondition`, `SqlExpression`, `ISubquery`, `TableReference`, `SortOrder`, `ExpressionAlias`, `CommonTableExpression`, and `DbSequence` moved from `SqlArtisan.Internal` to the root `SqlArtisan` namespace — dynamic-query idioms no longer need `using SqlArtisan.Internal;`. Binary-breaking — rebuild against this version. (#244)
### Fixed
...
- A builder chain is now single-use: calling a stage method or `Build()` after a successful `Build()` throws instead of silently contaminating the next statement. Rebuild the chain per call — e.g. a local function parameterized by the part that changes (see the pagination reference). (#245)
