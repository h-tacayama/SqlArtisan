...
### Tests
- Extended the integration matrix to cover data-value and DML-shape paths with no real-database execution: binding a `DateTime` / `decimal` / `bool` value as a parameter in `WHERE`, a `false` boolean round-trip, `COUNT(DISTINCT ...)`, multi-column `GROUP BY`, and an expression-valued `UPDATE ... SET` (`age = age + 1`). The two boolean cases are skipped on Oracle (Oracle XE has no native boolean type).
- Extended the integration matrix to cover the library API surface that had no real-database execution: the async Dapper path (`ExecuteAsync` / `ExecuteScalarAsync` / `QueryAsync`, and Oracle `ExecuteReturningIntoAsync`), the ad-hoc `DbTable` reference, and the thin Dapper passthroughs (`QuerySingle` / `QueryFirst` / `QueryFirstOrDefault` / `ExecuteReader` / `QueryMultiple`).
- Extended the integration matrix to execute the WHERE predicates, ordering, and clause variants that previously ran only in the exact-SQL unit tests: `BETWEEN`/`NOT BETWEEN`, `LIKE`/`NOT LIKE`, `IS NULL`/`IS NOT NULL`, `NOT IN` (list and subquery), logical `NOT`, and `ORDER BY ... DESC` (asserted on every engine); plus `NULLS FIRST`, `LIKE ... ESCAPE`, `OUTER APPLY`, `CROSS`/`LEFT JOIN LATERAL`, and SQL Server's `MERGE ... WHEN NOT MATCHED BY SOURCE` in the per-engine statement smoke.
- Extended the integration matrix to execute the join and set-operator constructs that previously ran only in the exact-SQL unit tests: `LEFT JOIN`, `CROSS JOIN`, and `SELECT DISTINCT` (asserted on every engine), plus `RIGHT JOIN`, `FULL JOIN` (every engine except MySQL), `INTERSECT`, and `INTERSECT ALL` (PostgreSQL / MySQL) in the per-engine statement smoke — closing the gap where only `INNER JOIN` and `UNION`/`EXCEPT` were exercised against a live database.
