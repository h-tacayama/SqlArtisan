...
## [Unreleased]
### Added
- Added support for joining a correlated derived table via `APPLY` / `LATERAL`: `CrossApply(subquery, schema)` / `OuterApply(subquery, schema)` (`CROSS APPLY` / `OUTER APPLY`, SQL Server / Oracle), `CrossJoinLateral(subquery, schema)` / `LeftJoinLateral(subquery, schema)` (`CROSS JOIN LATERAL` / `LEFT JOIN LATERAL ... ON true`, PostgreSQL / MySQL / Oracle), and `JoinLateral(subquery, schema).On(cond)` (`JOIN LATERAL ... ON ...`). The derived table is named by a `DerivedTableSchemaBase` handle, emitted bare to match CTE naming and referenced through itself rather than a repeated alias string: subclass it to expose typed `DbColumn` members, or use the inline `DerivedTable` and reference columns with `Column(...)` — by name, or type-safely from the projected `DbColumn` / SELECT-list `.As(...)` alias. CTEs mirror this — a `Cte` lets a one-off CTE skip declaring a `CteSchemaBase` subclass (use `Column(name)` for its columns). `APPLY` and `LATERAL` are distinct grammars for the same intent, so each is exposed as its own method per ADR 0002 — `Build(Dbms)` emits faithfully and never rewrites one form into the other; availability (SQLite supports neither; `LATERAL` has no SQL Server form) is left to the database and the analyzer (ADR 0003). (#122)
