...
## [Unreleased]
### Added
- Added `SQLA0006` and the `sqlartisan_target_version` (`<SqlArtisanTargetVersion>`) config key it reads: a build-time warning when a construct is supported on the target dialect but not until a *newer* engine version than the one you've declared — e.g. `MERGE` before PostgreSQL 15, `DATETRUNC` before SQL Server 2022. The matrix now carries an optional minimum-version floor per construct/dialect, sourced only from evidence already on record; declaring no version (or one that meets every bound) reproduces today's behavior exactly, and `sqlartisan_construct_*` overrides still keep the last word. A pinned `gvenzl/oracle-free:23-slim-faststart` integration lane live-proves the Oracle bounds against a real 23ai engine — it also caught a real `WITH RECURSIVE` gap (an `AS`-keyword requirement under Oracle's `RECURSIVE` grammar the matrix doesn't yet clear; see [Known limitations](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#known-limitations)) before it could ship as a false silence. See [Version-aware warnings](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#version-aware-warnings-sqla0006). (#263)
