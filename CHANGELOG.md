...
## [Unreleased]
### Added
- The schema-aware rules `SQLA0007`–`SQLA0010` ship in their own diagnostic category, `SqlArtisan.Schema`; `SQLA0001`–`SQLA0006` stay in `SqlArtisan.Dialect`. The two answer different questions — will this run on the engine you configured, versus does it agree with what your table classes say the columns are — so a bulk setting such as `dotnet_analyzer_diagnostic.category-SqlArtisan.Schema.severity = error` now reaches one family without the other. (#266)
- Added `SQLA0010`, an opt-in suggestion for `Count(column)` on a column the generated table class declares nullable — `COUNT(column)` counts the rows where that column is not NULL, which is correct SQL and sometimes exactly what you meant, but a surprise when you wanted the row count (`Count(Asterisk)`). Because it reports correct code it ships **disabled**: name it explicitly with `dotnet_diagnostic.SQLA0010.severity = suggestion` — a category-wide severity does not reach a rule that is disabled by default — and use `= warning` to see it in build or CI output, since a plain `dotnet build` prints nothing below warning level. Only the plain `Count(expr)` form is considered — `Count(Asterisk)` counts rows already, and `Count(Distinct, expr)` asks for distinct values, which `COUNT(*)` cannot give — and, like `SQLA0007`, it stays out of any statement with an outer join, where counting the column is precisely how you count the matched rows. See [Schema-aware warnings](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#schema-aware-warnings-sqla0007). (#266)
- Added `SQLA0009`, a build-time warning for an `INSERT` whose column list omits a column the generated table class declares `NOT NULL` with no default — a row the engine rejects unless something outside the catalog, such as a `BEFORE INSERT` trigger, supplies the value. A column it assigns itself (identity, auto-increment, generated, or one with a `DEFAULT`) is recorded as defaulted and never reported. Only the explicit-column-list form is checked: the positional `InsertInto(t).Values(...)` supplies every column by construction, and `InsertIgnoreInto` asked for error-raising rows to be skipped. **MySQL caveat:** outside strict mode MySQL substitutes an implicit default (`0`, `''`) and warns rather than rejecting the statement; `STRICT_TRANS_TABLES` is on by default from 5.7, so the warning matches the default configuration, and on a non-strict server it flags a column that will silently receive an implicit default instead of the value you meant. See [Schema-aware warnings](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#schema-aware-warnings-sqla0007). (#266)
- Added `SQLA0008`, a build-time warning for `NOT IN` over a subquery whose selected column is nullable — SQL's oldest trap, where a single NULL in the subquery makes the predicate NULL for every row and the query silently matches nothing. Reach for `NOT EXISTS`, or filter the NULLs out of the subquery. `IN` is unaffected and never reported. Like `SQLA0007` it reads the metadata `SqlArtisan.TableClassGen` emits, and it reads only a select list it can see: a subquery held in a variable, or one selecting anything but a single column, stays silent. See [Schema-aware warnings](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#schema-aware-warnings-sqla0007). (#266)
- Added `SQLA0007`, the first schema-aware warning: `IS NULL` / `IS NOT NULL` on a column the generated table class declares `NOT NULL`, where the predicate's answer is fixed before the query runs (`IS NULL` always false, `IS NOT NULL` always true). It reports only where it can see there is no outer join: the statement must visibly build its own query (a chain starting at `Select` / `Update` / `DeleteFrom` / `MergeInto` / `With` right there — one held in a variable, returned by a helper, or kept in a field is left alone) and must contain no `LeftJoin`, `RightJoin`, `FullJoin`, `NaturalLeftJoin`, `NaturalRightJoin`, `NaturalFullJoin`, `LeftJoinLateral`, or `OuterApply`. Past one of those a NOT NULL column is legitimately NULL on the null-supplied side, and `IS NULL` on it is the idiomatic anti-join; `InnerJoin` and `NaturalJoin` null-supply nothing and still report. It reads the `DbColumnMetadata` attributes `SqlArtisan.TableClassGen` emits, so it is silent for a hand-written table class, for a column whose nullability the tool could not determine, and for a column reached through `DbTable(...).Column(...)` — absence of the fact is absence of a claim, never a guess. Unlike the dialect rules this is not a dialect fact: the column's own declaration decides it on every engine. See [Schema-aware warnings](https://github.com/h-tacayama/SqlArtisan/blob/main/docs/analyzer.md#schema-aware-warnings-sqla0007). (#266)
