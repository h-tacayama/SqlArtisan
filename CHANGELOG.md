...
- **Breaking:** `SqlCondition` and `SqlExpression` moved from `SqlArtisan.Internal` to the root `SqlArtisan` namespace. Both are held and named by ordinary application code — a variable accumulating a dynamic `WHERE` clause, a helper method's return type — which previously forced an implementation-detail `using SqlArtisan.Internal;` just to name them. Every concrete node (`AndCondition`, `LikeCondition`, `CountFunction`, …) stays in `Internal`; only the two abstract roots move, and only code that named them with a fully qualified `SqlArtisan.Internal.SqlCondition` / `SqlArtisan.Internal.SqlExpression` needs a one-line fix — a bare `using SqlArtisan.Internal;` for these two types becomes unnecessary but harmless. (#244)
### Fixed
