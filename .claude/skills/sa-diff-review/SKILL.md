...
- A shape a **decided record** already covers — a `.claude/rules/` clause or
  an ADR clause that states the behaviour and why: cite the record and drop
  the candidate. A candidate that only *resembles* one is not covered: report
  it under its normal classification and let the record be cited in reply.
