...
this" suggestions, use `sa-diff-review-suggest` instead — it runs this skill in
...
here — it belongs to `sa-diff-review-suggest`'s improvement pass instead. If you
are running as `sa-diff-review-suggest`, that pass is part of the same run: hand
...
belongs to `sa-diff-review-suggest`, not here; this skill never reports one, not
...
- **Inaccuracy** — something the current code contradicts: a clause described
  ...
  preferred is not inaccurate — that's `sa-diff-review-suggest`'s territory.
...
- Any "better way to write this" suggestion with no rule/ADR/precedent to
  cite, for code or docs — run `sa-diff-review-suggest` for that pass instead.
...
non-defect improvement suggestions — re-run as `sa-diff-review-suggest` for those.
