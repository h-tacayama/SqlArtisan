...
- **Mutable builder fields returning `this`** are acceptable **only on a
  single-owner fluent-chain step**, protected by the #245 reuse contract (a
  stage call after `Build()` throws once the freeze-after-Build guard lands)
  — precedent: `TopClause.Percent`/`WithTies`, `StringAggFunction.WithinGroup`.
  Docs must never *promise* mutation there either (the contract wording is "a
  partial chain is one-way"). Flag the same shape on a **value-like node a
  caller might hold and derive from more than once** (an accumulator pattern —
  combining `SqlCondition`s with `&`/`|`, deriving `SortOrder` variants,
  anything similar): mutating the receiver there aliases or cross-contaminates
  the derivations, since both point at the same object. `AndCondition`/
  `OrCondition` and `SortOrder.NullsFirst`/`.NullsLast` are copy-on-write for
  exactly this reason — require the same of any new node shaped like them.
