...
public class DocsCalloutTests
{
    ...
    public static IEnumerable<object[]> Pages()
    {
        ...
        foreach (
            string path in Directory.EnumerateFiles(docsDir, "*.md", SearchOption.AllDirectories))
        {
        ...
        while (i < lines.Length)
        {
            ...
            if (kind == "NOTE")
            {
                Assert.False(
                    ...
                    $"{page}: [!NOTE] at line {start + 1} contains a table — tables are "
                        + $"WARNING-only.");
            }
        }
    }
    ...
    private static string FindRepoRoot()
    {
        ...
        if (dir is null)
        {
            throw new InvalidOperationException(
                "Could not find repo root (SqlArtisan.sln) above " + AppContext.BaseDirectory);
        }
