...
/// <summary>
/// The "coverage" half of the coverage/integrity gate pair (#93): every public member
/// user query code can reference has a <see cref="DialectMatrix"/> entry or a documented
/// exclusion here, so "silence = verified" holds; <c>op_*</c> operators are in (#219).
/// </summary>
public class DialectMatrixCoverageTests
{
    /// <summary>Members (keyed <c>Type.Member</c>) deliberately without a matrix entry, with the
    /// reason.</summary>
    private static readonly IReadOnlyDictionary<string, string> Exclusions =
    ...
    public void EveryReferencablePublicMember_HasMatrixEntryOrDocumentedExclusion()
    {
        ...
        Assert.True(
            uncovered.Count == 0,
            $"{uncovered.Count} public members have neither a matrix entry nor a documented "
                + $"exclusion:\n  "
    ...
    public void Exclusions_ResolveToRealMembersWithoutMatrixEntries()
    {
        ...
        HashSet<string> realMembers = [.. typeof(Sql).Assembly.GetExportedTypes()
            .SelectMany(type => type
                .GetMethods(BindingFlags.Public
                    | BindingFlags.Static | BindingFlags.Instance | BindingFlags.DeclaredOnly)
                .Cast<MemberInfo>()
                .Concat(type.GetProperties(BindingFlags.Public
                    | BindingFlags.Static | BindingFlags.Instance | BindingFlags.DeclaredOnly))
                .Concat(type.GetFields(BindingFlags.Public
                    | BindingFlags.Static | BindingFlags.Instance | BindingFlags.DeclaredOnly))
        ...
        Assert.True(
            stale.Count == 0,
            $"{stale.Count} exclusions are stale (member gone) or contradicted by a "
                + $"matrix entry:\n  "
                + string.Join("\n  ", stale));
    }
