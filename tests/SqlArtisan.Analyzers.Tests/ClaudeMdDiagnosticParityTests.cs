...
{
    private static readonly Regex BulletIdPattern = new(
        @"^- \*\*(SQLA\d{4})\*\*",
    ...
    private static readonly IReadOnlyList<string> ShippedIds =
        [.. new DialectUsageAnalyzer().SupportedDiagnostics.Select(d =>
            d.Id).Distinct(StringComparer.Ordinal)];

    [Fact]
    public void EveryShippedId_HasABulletInClaudeMd()
    {
        string[] missing = [.. ShippedIds.Except(DocumentedIds, StringComparer.Ordinal).OrderBy(id
            => id, StringComparer.Ordinal)];

        Assert.True(
            missing.Length == 0,
            $"{missing.Length} shipped diagnostic id(s) have no '## Analyzer' bullet in "
                + $"CLAUDE.md:\n  "
                + string.Join("\n  ", missing));
    }
    ...
    {
        string[] phantom = [.. DocumentedIds.Except(ShippedIds, StringComparer.Ordinal).OrderBy(id
            => id, StringComparer.Ordinal)];

        Assert.True(
            phantom.Length == 0,
            $"{phantom.Length} id(s) bulleted in CLAUDE.md's '## Analyzer' section name "
                + $"no diagnostic "
    ...
    public void ShipsCount_MatchesTheDistinctIdCount()
    {
        ...
        Assert.True(
            match.Success,
            "CLAUDE.md's '## Analyzer' section has no 'ships <N> diagnostics' sentence to check.");

        Assert.True(
            NumberWords.TryGetValue(match.Groups[1].Value, out int claimed),
            $"'{match.Groups[1].Value}' in CLAUDE.md's 'ships {match.Groups[1].Value} "
                + $"diagnostics' is not a "
        ...
        Assert.True(
            ...
            $"CLAUDE.md claims '{match.Groups[1].Value}' ({claimed}) diagnostics; "
                + $"DialectUsageAnalyzer.SupportedDiagnostics carries {ShippedIds.Count} "
                    + $"distinct ids.");
    }
